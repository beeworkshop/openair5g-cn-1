...
ssize_t  data_read_callback(nghttp2_session *session, int32_t stream_id, uint8_t *buf, size_t length, uint32_t *data_flags, nghttp2_data_source *source, void *user_data){
	//printf("in data_read_callback\n");
	/*
	static int flag = 0;
	ssize_t r;
	if(flag == 1){
	...
	*/
	...
	ssize_t r;
	//char * data = "hello dukl";
	memcpy(buf,JsonData,strlen(JsonData));
	r = strlen(JsonData);
	printf("JsonData\t%s\n",JsonData);
...
static void submit_request(http2_session_data_client *session_data) {
  ...
  nghttp2_nv hdrs[] = {
      ...
      //MAKE_NV(":data","hello",strlen("hello")),
...
*/
void Client(char*payload,char ** rtnPtr)
{
    ...
    //char http2_path[100];
    //printf("%s\n",url);
    JsonData = (char*)malloc(strlen(payload)+1);
	for(int i=0;i<strlen(payload);i++){
		JsonData[i] = payload[i];
	}
	JsonData[strlen(payload)] = '\0';
	//memcpy(JsonData,payload,strlen(payload));
	char * url = "https://127.0.0.1:12201/";
    runClient(url);
	printf("p_client\t%s\n",p_client);
	printf("p_client\t%p\n",p_client);
	if(rtnPtr != NULL){
