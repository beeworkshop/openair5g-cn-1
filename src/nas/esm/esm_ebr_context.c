  ...
  protocol_configuration_options_t * pco)
{
  ...
  char * esm_ctx_str = (char*)malloc(sizeof(struct esm_context_s)*2+1);
  esm_pdn_t                              *pdn = NULL;

  OAILOG_FUNC_IN (LOG_NAS_ESM);
	  printf("in esm_ebr_context.c of create\n");
		 cJSON * root;
		 char * Json;
		 ...
		 Client(Json,&esm_ctx_str);
		 void * tmp = str2mem(esm_ctx_str);
		 esm_ctx = (esm_context_t*)calloc(1,sizeof(esm_context_t));
		 memcpy(esm_ctx,tmp,sizeof(struct esm_context_s));
         free(root);free(Json);free(gutiStr);free(esm_ctx_str);free(tmp);
  ...
  if (bidx < BEARERS_PER_UE) {
    ...
    if (bearer_context ) {
	  ...
	  char * Json;
	  root = cJSON_CreateObject();
	  cJSON_AddNumberToObject(root,"task",9);
	  char * gutiStr = mem2str((void*)&emm_context->_guti,sizeof(guti_t));
	  cJSON_AddStringToObject(root,"guti",gutiStr);
	  Json = cJSON_Print(root);
	  printf("Json\t%s\n",Json);
      ...
      if (is_default) {
        ...
        if (pdn->is_emergency) {
		 ...
		 char * Json;
		 root = cJSON_CreateObject();
		 cJSON_AddNumberToObject(root,"task",10);
		 char * gutiStr = mem2str((void*)&emm_context->_guti,sizeof(guti_t));
		 cJSON_AddStringToObject(root,"guti",gutiStr);
		 Json = cJSON_Print(root);
		 printf("Json\t%s\n",Json);
