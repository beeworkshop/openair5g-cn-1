...
static int _emm_cn_pdn_config_res (emm_cn_pdn_config_res_t * msg_pP)
{
    ...
    printf("in emm_cn.c of res\n");
	char * apn_esm_data_str = (char*)calloc(1,sizeof(struct esm_proc_data_s)*2+50);
	cJSON * root;
	char * Json;
	root = cJSON_CreateObject();
	cJSON_AddNumberToObject(root,"task",15);
	char * gutiStr = mem2str((void*)&emm_ctx->_guti,sizeof(guti_t));
	cJSON_AddStringToObject(root,"guti",gutiStr);
	Json = cJSON_Print(root);
	Client(Json,&apn_esm_data_str);
	printf("%s\n",apn_esm_data_str);


	int isApnNull = 0;
	int apn_mlen,apn_slen,apn_data_len;
    char * apn_data_str; 
    char * esm_data_str;
	bstring apn;
	...
	cJSON * json = cJSON_Parse(apn_esm_data_str);
	for(cJSON * tmp = json->child;tmp!=NULL;tmp=tmp->next){
		if(memcmp("isApnNull",tmp->string,sizeof("isApnNull"))==0){
			isApnNull = tmp->valueint;
		}else if(memcmp("apn_mlen",tmp->string,sizeof("apn_mlen"))==0){
			apn_mlen = tmp->valueint;
		}else if(memcmp("apn_slen",tmp->string,sizeof("apn_slen"))==0){
			apn_mlen = tmp->valueint;
		}else if(memcmp("apn_data",tmp->string,sizeof("apn_data"))==0){
			apn_data_str = (char*)calloc(1,strlen(tmp->valuestring)+1);
			memcpy(apn_data_str,tmp->valuestring,strlen(tmp->valuestring));
		}else if(memcmp("apn_data_length",tmp->string,sizeof("apn_data_length"))==0){
			apn_data_len = tmp->valueint;
		}else if(memcmp("esm_proc_data",tmp->string,sizeof("esm_proc_data"))==0){
			esm_data_str = (char*)calloc(1,strlen(tmp->valuestring)+1);
    ...
    struct apn_configuration_s* apn_config = mme_app_select_apn(ue_mm_context, esm_data->apn);//需传递apn content
        if (!apn_config) {
            /*
        ...
        if (pdn_cid < MAX_APN_PER_UE) {
			...
			is_esm_data_mdfy = 1;
            esm_data->pdn_cid = pdn_cid;
            esm_data->bearer_qos.qci       = apn_config->subscribed_qos.qci;
            esm_data->bearer_qos.pci       = apn_config->subscribed_qos.allocation_retention_priority.pre_emp_capability;
            esm_data->bearer_qos.pl        = apn_config->subscribed_qos.allocation_retention_priority.priority_level;
            esm_data->bearer_qos.pvi       = apn_config->subscribed_qos.allocation_retention_priority.pre_emp_vulnerability;
            esm_data->bearer_qos.gbr.br_ul = 0;
            esm_data->bearer_qos.gbr.br_dl = 0;
            esm_data->bearer_qos.mbr.br_ul = 0;
            esm_data->bearer_qos.mbr.br_dl = 0;

            rc = esm_proc_pdn_connectivity_request (emm_ctx,
                    esm_data->pti,
                    esm_data->pdn_cid,
                    apn_config->context_identifier,
                    esm_data->request_type,
                    esm_data->apn,
                    esm_data->pdn_type,
                    esm_data->pdn_addr,
                    &esm_data->bearer_qos,
                    (esm_data->pco.num_protocol_or_container_id ) ? &esm_data->pco:NULL,
                    &esm_cause);

            if (rc != RETURNerror) {
                if ((!is_pdn_connectivity) || ((is_pdn_connectivity) && (EPS_BEARER_IDENTITY_UNASSIGNED == ue_mm_context->pdn_contexts[pdn_cid]->default_ebi))) {
                    rc = esm_proc_default_eps_bearer_context (emm_ctx, esm_data->pti, pdn_cid, &new_ebi, esm_data->bearer_qos.qci, &esm_cause);
                }
            ...
            if (!is_pdn_connectivity) {
                nas_itti_pdn_connectivity_req (esm_data->pti, msg_pP->ue_id, pdn_cid, &emm_ctx->_imsi,
                        esm_data, esm_data->request_type);
            } else {
        ...
        OAILOG_FUNC_RETURN (LOG_NAS_EMM, RETURNerror);
{
	...
	char * Json;
	root = cJSON_CreateObject();
	cJSON_AddNumberToObject(root,"task",11);
	char * gutiStr = mem2str((void*)&emm_ctx->_guti,sizeof(guti_t));
	cJSON_AddStringToObject(root,"guti",gutiStr);
	char * esm_rtn_str = mem2str((void*)esm_data,sizeof(struct esm_context_s));
	cJSON_AddStringToObject(root,"esm_data",esm_rtn_str);
	cJSON_AddNumberToObject(root,"is_esm_data_mdfy",is_esm_data_mdfy);
	Json = cJSON_Print(root);
	Client(Json,NULL);

	free(root);free(Json);free(gutiStr);free(esm_rtn_str);free(esm_data);free(apn);
}
}
